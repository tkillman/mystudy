...
class game_Frame extends JFrame implements KeyListener, Runnable {
	...
	public void init() {
		...
		Enemy_img = new ImageIcon("image/enemy.png").getImage();
		// 이미지 만드는 방식을 ImageIcon으로 변경.

		Player_img = new Image[5];
		for (int i = 0; i < Player_img.length; ++i) {
			Player_img[i] = new ImageIcon("image/f15k_" + i + ".png").getImage();
		}
		...
		for (int i = 0; i < Cloud_img.length; ++i) {
			Cloud_img[i] = new ImageIcon("image/cloud_" + i + ".png").getImage();
		}
		// 구름을 3개 동시에 그리는데 편의상 배열로 3개를 동시에 받는다.

		Explo_img = new Image[3];
		for (int i = 0; i < Explo_img.length; ++i) {
			Explo_img[i] = new ImageIcon("image/explo_" + i + ".png").getImage();
		}
		// 폭발 애니메이션 표현을 위해
